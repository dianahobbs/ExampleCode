# Title: ADNI Cohort Tau/Age Regional Analyses for Brian
...
#clear memory
#READ IN DATA
tauage = read.csv("./neuropsych_baseline.filter.av45.tau_n407.csv")%>%
  filter(av45_APos_cross == "A-") # n = 220
...
# Create a 'for' loop 
for (i in 1:ncol(roi)){
  region_fit <- lm(roi[,i] ~ sex+av45_cross.sectional+race.binary+age, data=tauage)
  robust_fit <- rlm(roi[,i] ~ sex+av45_cross.sectional+race.binary+age, data=tauage)
  region_fit_int <- lm(roi[,i] ~ sex+av45_cross.sectional+race.binary+age+age*sex, data=tauage)
  robust_fit_int <- rlm(roi[,i] ~ sex+av45_cross.sectional+race.binary+age+age*sex, data=tauage)
  ...
  p_robust=f.robftest(robust_fit, var = "av45_cross.sectional"); model_robust[outcount, 3]=p_robust$p.value
  ...
  p_robust_int=f.robftest(robust_fit_int, var = "av45_cross.sectional"); model_robust_int[outcount, 3]=p_robust_int$p.value
...
colnames(PVal_robust) <- c("(Intercept)","sexFemale","av45_cross.sectional","race.binary1","age")
# region_fit_int.COF
...
colnames(PVal_robust_int) <- c("(Intercept)","sexFemale","av45_cross.sectional","race.binary1","age","sexFemale:age")
...
centiloid_region <- data.frame("B"=Estimate_region$av45_cross.sectional,"t"=TVal_region$av45_cross.sectional,"p"=PVal_region$av45_cross.sectional,"p_corr"=(p.adjust(PVal_region$av45_cross.sectional, method = "BH",n = length(PVal_region$av45_cross.sectional))))
...
centiloid_robust <- data.frame("B"=Estimate_robust$av45_cross.sectional,"t"=TVal_robust$av45_cross.sectional,"p"=PVal_robust$av45_cross.sectional,"p_corr"=(p.adjust(PVal_robust$av45_cross.sectional, method = "BH",n = length(PVal_robust$av45_cross.sectional))))
...
centiloid_region_int <- data.frame("B"=Estimate_region_int$av45_cross.sectional,"t"=TVal_region_int$av45_cross.sectional,"p"=PVal_region_int$av45_cross.sectional,"p_corr"=(p.adjust(PVal_region_int$av45_cross.sectional, method = "BH",n = length(PVal_region_int$av45_cross.sectional))))
...
centiloid_robust_int <- data.frame("B"=Estimate_robust_int$av45_cross.sectional,"t"=TVal_robust_int$av45_cross.sectional,"p"=PVal_robust_int$av45_cross.sectional,"p_corr"=(p.adjust(PVal_robust_int$av45_cross.sectional, method = "BH",n = length(PVal_robust_int$av45_cross.sectional))))
...
# region: cortical(0) - 0
#         subcortical(0) - pallidum, putamen 
# robust: cortical(1) - inferior temporal 
#         subcortical(3)- pallidum, putamen, amygdala
...
age_amy=ggplot(tauage, aes(x=age, y=amygdala, linetype=sex )) +
  xlab("Age") + ylab("Amygdala Tau")+  scatter_theme + theme(axis.title.x = element_blank())
...
# region: cortical(15) - frontal pole, lateral occipital, pars orbitalis, rostral middle frontal, postcentral
#         subcortical(0) - 0
# robust: cortical(17) - frontal pole, lateral occipital, pars orbitalis, rostral middle frontal, postcentral
#         subcortical(0)- 0
...
sex_parsorbit=ggplot(tauage, aes(x=sex, y=pars.orbitalis, fill=sex))+ ylab("Pars Orbitalis Tau")+  violin_theme  
sex_rostmidfron=ggplot(tauage, aes(x=sex, y=rostral.middle.frontal, fill=sex))+ ylab("Rostral Middle Frontal Tau")+  violin_theme  
sex_postcent=ggplot(tauage, aes(x=sex, y=postcentral, fill=sex))+ ylab("Postcentral Tau")+  violin_theme  


##### Merge panels into figures for the paper using cowplot ####
scatter_age=plot_grid(age_inftemp,age_pallidum,age_putamen,age_amy, ncol=2, labels = c("A)", "B)", "C)","D)"), align = "v", axis = "bt")+theme(plot.background = element_rect(fill = "white"), panel.border = element_blank())
tiff('./Figures/Scatter_age.tiff', units="in", width=8.5, height=8.5, res=200,type="cairo" )
print(scatter_age)
dev.off()

violin_sex=plot_grid(sex_frontpole,sex_latocc,sex_parsorbit,sex_rostmidfron,sex_postcent, ncol=2, labels = c("A)","B)","C)","D)","E)"), align = "v", axis = "bt")+theme(plot.background = element_rect(fill = NA), panel.border = element_blank())
...
##### Cognitive scores ##### 
cog <- tauage%>%dplyr::select(RID,age,av45_APos_cross,cdr,ANIMALS,tmb)%>% #n = 220
  na.omit()

control_cog <- cog%>%filter(av45_APos_cross=="A-" & cdr==0) # n = 147
...
composite <- cog%>%dplyr::select(RID,composite)
data <- merge(tauage, composite, by="RID") # n = 220
# Significant region
