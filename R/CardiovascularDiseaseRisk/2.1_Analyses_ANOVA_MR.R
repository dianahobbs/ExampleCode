...
mri$SEX <- as.factor(mri$SEX); mri$SEX <- relevel(mri$SEX, "Male") 
mri$APOE4 <- as.factor(mri$APOE4); mri$APOE4 <- relevel(mri$APOE4, "0")
mri$CVD <- as.factor(mri$CVD); mri$CVD <- relevel(mri$CVD, "0")
...
# Create a 'for' loop 
for (i in 1:ncol(ROI)){
  model <- anova(lm(ROI[,i] ~ AGE + SEX + APOE4 + followup_time + CVD + Centiloid + CVD*Centiloid, data=mri))
  MODEL[[i]] <- model
  names(MODEL)[i] <- colnames(ROI)[i]
}
write.csv(MODEL,"./Rabin_Replication_Output/MR/Colsp_Anova_1.csv")
rm(model,MODEL,i,mri,ROI)

###### Organize loop output in usable format 
Data <- read.csv("./Rabin_Replication_Output/MR/Colsp_Anova_1.csv")
...
# for combined figure 
MR_CortCVD.f=ggseg(.data=CortCVD,mapping=aes(fill=f),position = "stacked", colour="black",size=.5, hemisphere="left")+
  labs(title="MRI") +  custom_theme
MR_CortCVD.Sig.f=ggseg(.data=CortCVD.Sig,mapping=aes(fill=f),position = "stacked", colour="black",size=.5, hemisphere="left")+custom_theme
MR_SubCortCVD.f=ggseg(.data=SubcortCVD,mapping=aes(fill=f),position = "stacked", colour="black",size=.5, hemisphere="left", atlas=aseg)+
  labs(title="MRI") +  custom_theme_sub
MR_SubCortCVD.Sig.f=ggseg(.data=SubcortCVD.Sig,mapping=aes(fill=f),position = "stacked", colour="black",size=.5, hemisphere="left", atlas=aseg)+custom_theme_sub
...
# Save Figure
tiff('./Rabin_Replication_Output/MR/Colsp_Anova_Unthresholded_1.tiff', units="in", width=7, height=9, res=200,type="cairo" )
print(merged_brain)
# For combined figure 
Colsp_MRUnthresh_cvd_row=plot_grid(MR_CortCVD.f, align="hv", axis="tb", rel_widths = c(1, .3))+theme(plot.background = element_rect(fill = "black") )  
Colsp_MR_row=plot_grid(MR_CortCVD.f, MR_CortCVD.Sig.f)  
dev.off()
Colsp_MRUnthresh_cvd_row_SubCort=plot_grid(MR_SubCortCVD.f, align="hv", axis="tb", rel_widths = c(1, .3))+theme(plot.background = element_rect(fill = "black") )  
Colsp_MR_row_SubCort=plot_grid(MR_SubCortCVD.f, MR_SubCortCVD.Sig.f)  
dev.off()
...
# Save Figure
tiff('./Rabin_Replication_Output/MR/Colsp_Anova_Thresholded_1.tiff', units="in", width=7, height=9, res=200,type="cairo")
print(merged_brain.sig)
# For combined figure
Colsp_MRThresh_cvd_row=plot_grid(MR_CortCVD.Sig.f, align="hv", axis="tb", rel_widths = c(1, .3))+theme(plot.background = element_rect(fill = "black") )  
dev.off()
Colsp_MRThresh_cvd_row_SubCort=plot_grid(MR_SubCortCVD.Sig.f, align="hv", axis="tb", rel_widths = c(1, .3))+theme(plot.background = element_rect(fill = "black") )  
dev.off()
