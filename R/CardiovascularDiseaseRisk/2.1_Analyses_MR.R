...
# Create a 'for' loop 
for (i in 1:ncol(ROI)){
  lm <- lm(ROI[,i] ~ AGE + SEX + APOE4 + followup_time + CVD + Centiloid + CVD*Centiloid, data=mri)
  rlm <- rlm(ROI[,i] ~ AGE + SEX + APOE4 + followup_time + CVD + Centiloid + CVD*Centiloid, data=mri)
  lm.sum <- summary(lm)
  rlm.sum <- summary(rlm)
...
write.csv(LM.COF,"./Rabin_Replication_Output/MR/Colsp_LM.COF_1.csv")
write.csv(RLM.COF,"./Rabin_Replication_Output/MR/Colsp_RLM.COF_1.csv")
rm(lm,lm.cof,LM.COF,lm.sum,rlm,rlm.cof,RLM.COF,rlm.sum,i,ROI,rob_p,outcount,mri,region)

###### Organize loop output in usable format 
Data <- read.csv("./Rabin_Replication_Output/MR/Colsp_LM.COF_1.csv")
...
# Save Figure
tiff('./Rabin_Replication_Output/MR/Colsp_Unthresholded_1.tiff', units="in", width=7, height=9, res=200,type="cairo" )
print(merged_brain)
dev.off()
...
# Save Figure
tiff('./Rabin_Replication_Output/MR/Colsp_Thresholded_1.tiff', units="in", width=7, height=9, res=200,type="cairo")
...
###### Robust regression ######
Data <- read.csv("./Rabin_Replication_Output/MR/Colsp_RLM.COF_1.csv")
# Extract Estimates 
...
# Save Figure
tiff('./Rabin_Replication_Output/MR/Colsp_Unthresholded_rob_1.tiff', units="in", width=7, height=9, res=200,type="cairo" )
...
# Save Figure
tiff('./Rabin_Replication_Output/MR/Colsp_Thresholded_rob_1.tiff', units="in", width=7, height=9, res=200,type="cairo")
