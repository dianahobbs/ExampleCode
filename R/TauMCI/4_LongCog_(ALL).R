...
cogn_long$Biomarker_group_EC <- as.factor(cogn_long$Biomarker_group_EC)
cogn_long$Biomarker_group_IT <- as.factor(cogn_long$Biomarker_group_IT)
cogn_long$Biomarker_group_metaROI <- as.factor(cogn_long$Biomarker_group_metaROI)
cogn_long$Biomarker_group_ANY <- as.factor(cogn_long$Biomarker_group_ANY)

####### All stat models exclude A-T+ participant/s 
...
write.csv(coef(Longcogn_PACC)$ID, './5_25_22/long2_coefficients_PACC_YrBl_6-14.csv', row.names = TRUE)


###### LME models for statistical reporting

#Run lme between pathology groups (regardless of MCI status)
data_group124 <- cogn_long[which(cogn_long$Biomarker_group_metaROI!="3"),]

Longcogn_PACC_bygroup <- lmer (PACC ~ YrBl*Biomarker_group_metaROI +(1 + YrBl|ID) + AGE + EDUC + SEX, data=data_group124, contrasts=c("contr.sum", "contr.poly"), REML=FALSE)
summary(Longcogn_PACC_bygroup)


residualplot<-plot(Longcogn_PACC_bygroup, type = c("p", "smooth"))
print(residualplot) 


data_group124 <- cogn_long[which(cogn_long$Biomarker_group_EC!="3"),]

Longcogn_PACC_bygroup_EC <- lmer (PACC ~ YrBl*Biomarker_group_EC +(1 + YrBl|ID) + AGE + EDUC + SEX, data=data_group124, contrasts=c("contr.sum", "contr.poly"), REML=FALSE)
summary(Longcogn_PACC_bygroup_EC)

data_group124 <- cogn_long[which(cogn_long$Biomarker_group_IT!="3"),]

Longcogn_PACC_bygroup_IT <- lmer (PACC ~ YrBl*Biomarker_group_IT +(1 + YrBl|ID) + AGE + EDUC + SEX, data=data_group124, contrasts=c("contr.sum", "contr.poly"), REML=FALSE)
summary(Longcogn_PACC_bygroup_IT)
...
###### PLOTS
# Main figure: Plot by metaROI biomarker group all on same figure, not separating by MCI status
Longcogn_PACC_bygroup <- lmer (PACC ~ YrBl*Biomarker_group_metaROI +(1 + YrBl|ID) + AGE + EDUC + SEX, data=cogn_long, contrasts=c("contr.sum", "contr.poly"), REML=FALSE)
summary(Longcogn_PACC_bygroup)
...
library(tidyverse)
library(grid)
fit_mod <- tibble(fitted(Longcogn_PACC_bygroup))
#temp <- as.matrix(fitted(Longcogn_PACC_bygroup))
fit_mod <- cbind(fit_mod, ID = cogn_long$ID) 
fit_mod <- cbind(fit_mod, BaseCog_Visit = cogn_long$BaseCog_Visit)
#library(splitstackshape)
#fit_mod <- cSplit(fit_mod, 'ID', sep=".", type.convert=FALSE)
#colnames(fit_mod)[colnames(fit_mod)=="ID_1"] <- "ID"
#colnames(fit_mod)[colnames(fit_mod)=="ID_2"] <- "time"
#fit_mod$time <- as.integer(fit_mod$time)
Longcogn_fit = merge(fit_mod, cogn_long, by=c("ID","BaseCog_Visit"))

Logncogn_PACC_LME_spaghetti_bygroup <- Longcogn_fit %>%
  ggplot(aes(x=YrBl, y=`fitted(Longcogn_PACC_bygroup)`, color=Biomarker_group_metaROI)) +
  geom_smooth(method=lm, se=TRUE, aes(fill=Biomarker_group_metaROI), show.legend =FALSE) +
  geom_smooth(method=lm, se=FALSE, aes(fill=Biomarker_group_metaROI)) +
  #geom_rug(aes(x = YrBl), sides="b") +
  scale_color_manual(values = c("#FC8D62", "#E78AC3", "#66C2A5", "#FFD92F", "black"), labels = c("A+T+", "A+T-", "A-T+", "A-T-")) + scale_fill_manual(values = c("#FC8D62", "#E78AC3", "#66C2A5", "#FFD92F", "black")) + 
  labs(x="Visit (Years)", y="PACC", title = 'Knight ADRC')+
  theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "gray94", size = 0.25), axis.line = element_line(colour = "black"),
        text = element_text(size = 24), plot.title = element_text(hjust = 0.5, size = 28),
  ...
  ggplot2::scale_x_continuous(limits=c(0, 16), breaks = c(0, 5, 10, 15))+
  ggplot2::scale_y_continuous(limits=c(-5, 3), breaks = c(-4, -2, 0, 2))+
  guides(fill=F)

print(Logncogn_PACC_LME_spaghetti_bygroup) 

ggsave("./5_25_22/6-14_long_cogn2_ADRC_metaROI_wcovariates_YrBl.png", plot = Logncogn_PACC_LME_spaghetti_bygroup, device = "png", width = 20, height = 20, units = "cm", dpi = 500)
...
cogn_group1<- cogn_long[which(cogn_long$Biomarker_group_metaROI=="1"),]
...
ggsave("./5_25_22/6-14_long_cogn_ADRC_spaghetti_group1_bystatus.png", plot = Logncogn_PACC_LME_spaghetti_group1_bystatus, device = "png", width = 80, height = 20, units = "cm", dpi = 500)

#select just those in group 2 (i.e., A+T-)

cogn_group2<- cogn_long[which(cogn_long$Biomarker_group_metaROI=="2"),]
...
ggsave("./5_25_22/6-14_long_cogn_ADRC_spaghetti_group2_bystatus.png", plot = Logncogn_PACC_LME_spaghetti_group2_bystatus, device = "png", width = 80, height = 20, units = "cm", dpi = 500)
...
cogn_group3<- cogn_long[which(cogn_long$Biomarker_group_metaROI=="3"),]
...
ggsave("./5_25_22/6-14_long_cogn_ADRC_group3_line.png", plot = Logncogn_PACC_group3_line, device = "png", width = 80, height = 20, units = "cm", dpi = 500)
...
ggsave("./5_25_22/6-14_long_cogn_ADRC_spaghetti_group3.png", plot = Logncogn_PACC_LME_spaghetti_group3, device = "png", width = 80, height = 20, units = "cm", dpi = 500)


#select just those in group 4 (i.e., A-T-)

cogn_group4<- cogn_long[which(cogn_long$Biomarker_group_metaROI=="4"),]
...
ggsave("./5_25_22/6-14_long_cogn_ADRC_spaghetti_group4_bystatus.png", plot = Logncogn_PACC_LME_spaghetti_group4_bystatus, device = "png", width = 80, height = 20, units = "cm", dpi = 500)
...
ggsave("./5_25_22/6-14_long_cogn2_ADRC_metaROI_wcovariates_bymci_YrBl_NEW.png", plot = longcogn_ADRC, device = "png", width = 80, height = 20, units = "cm", dpi = 500)
